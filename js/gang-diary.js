...
function startRecording() {
  ...
  recognition.onstart = () => {
    ...
    if (elements.transcriptionText) elements.transcriptionText.textContent = "🎙️ Listening in Telugu...";
  };
  ...
  recognition.onerror = (event) => {
    ...
    if (event.error === "network") {
      ...
      showToast("🎙️ Please allow microphone access", "warn");
    }
  };
...
function updateRecordBtn(recording) {
  ...
  if (elements.micEmoji) elements.micEmoji.textContent = recording ? "⏹️" : "🎙️";
}
...
async function generateDiaryEntry() {
  ...
  if (!text) { showToast("🎙️ Please record or type work details first", "warn"); return; }
